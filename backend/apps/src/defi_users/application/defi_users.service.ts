...
export class DefiUsersService {
    ...
    async createDefiUser(createDefiUserDto: CreateDefiUserDto): Promise<DefiUser> {
    ...
    async getDefiUserByWalletAddress(wallet_address: string): Promise<DefiUser | null> {
        const defiUser = await this.defiUsersRepository.getDefiUserByWalletAddress(wallet_address);

        if (!defiUser) {
            throw new NotFoundException(`DefiUser with wallet address ${wallet_address} not found`);
        }
